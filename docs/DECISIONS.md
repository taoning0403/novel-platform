...
| [0003 — Client-supplied LLM credentials](adr/0003-client-supplied-llm-credentials.md) | Proposed for a future phase | Add no LLM calls or secret storage in v0.1.0; design credential lifetime, redaction, transport, persistence, and provider boundaries before integration. |
...
| [0010 — Local library storage and atomic import](adr/0010-local-library-storage-and-atomic-import.md) | Accepted | Keep Edition identity separate from file revisions; inspect into private temporary storage, atomically commit random-key local files with database rows, and back up PostgreSQL and files together. |
...
| [0012 — Credential, device, Passkey, recovery, and session authentication](adr/0012-credential-device-passkey-and-recovery-authentication.md) | Accepted | Replace daily passwords and browser setup with rotatable reader credentials, server-secret-backed Device authorization, administrator Passkeys, restricted CLI recovery, and database-checked Sessions. |
| [0013 — Single-administrator library ownership and invited-reader visibility](adr/0013-single-admin-library-and-reader-visibility.md) | Accepted | Keep explicit administrator ownership while separating manageable content from filtered reader-visible content and requiring an operator-selected legacy conversion. |
...
1. Add the next numbered file under `docs/adr/` (currently `0016`).
